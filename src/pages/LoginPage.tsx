...
export const LoginPage = () => {
  ...
  const [step, setStep] = useState<LoginStep>(() =>
    hasUsableAuthState(storedAuth) && !storedAuth?.selectedStore
      ? "store"
      : "login",
  );
  ...
  if (step === "store" && authDraft) {
    return (
      <main className="auth-shell is-store-select">
        ...
        <section
          className="store-select-panel"
          aria-label={t("storeSelectTitle")}>
  ...
  return (
    <main className="auth-shell">
      ...
      <section
        ...
        <form
          ...
          <button
            ...
            onClick={() => {
              setMode((currentMode) =>
                currentMode === "code" ? "password" : "code",
              );
              setMessage("");
              setError("");
              resetTurnstile();
      ...
      <label className="auth-terms">
        ...
        <a
          href="https://biz.bello2u.com/privacy"
          target="_blank"
          rel="noreferrer">
          {t("termsOfService")}
        </a>
...
const AuthBrand = ({ showText = false }: { showText?: boolean }) => {
  return (
